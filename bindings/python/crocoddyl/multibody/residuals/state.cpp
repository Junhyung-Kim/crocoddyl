...
void exposeResidualFlyState2() {
  bp::register_ptr_to_python<boost::shared_ptr<ResidualFlyState2> >();

  bp::class_<ResidualFlyState2, bp::bases<ResidualModelAbstract> >(
      "ResidualFlyState2",
      "This cost function defines a residual vector as r = x - xref, with x and xref as the current and reference\n"
      "state, respectively.",
      bp::init<boost::shared_ptr<StateAbstract>, Eigen::VectorXd, std::size_t>(
      ...
      .def(bp::init<boost::shared_ptr<StateAbstract> >(
          ...
          "The default reference state is obtained from state.zero(), and nu from state.nv.\n"
          ":param state: state description"))
      .def<void (ResidualFlyState2::*)(const boost::shared_ptr<ResidualDataAbstract>&,
                                        const Eigen::Ref<const Eigen::VectorXd>&,
                                        const Eigen::Ref<const Eigen::VectorXd>&)>(
          "calc", &ResidualFlyState2::calc, bp::args("self", "data", "x", "u"),
          "Compute the state cost.\n\n"
          ":param data: cost data\n"
          ":param x: state point (dim. state.nx)\n"
          ":param u: control input (dim. nu)")
      ...
      .def<void (ResidualFlyState2::*)(const boost::shared_ptr<ResidualDataAbstract>&,
                                        const Eigen::Ref<const Eigen::VectorXd>&,
                                        const Eigen::Ref<const Eigen::VectorXd>&)>(
          "calcDiff", &ResidualFlyState2::calcDiff, bp::args("self", "data", "x", "u"),
          "Compute the derivatives of the state cost.\n\n"
          "It assumes that calc has been run first.\n"
          ":param data: action data\n"
          ":param x: state point (dim. state.nx)\n"
      ...
      .def<void (ResidualFlyState2::*)(const boost::shared_ptr<ResidualDataAbstract>&,
          ...
          "calcDiff", &ResidualModelAbstract::calcDiff, bp::args("self", "data", "x"))
      .add_property("reference",
                    bp::make_function(&ResidualFlyState2::get_reference, bp::return_internal_reference<>()),
                    &ResidualFlyState2::set_reference, "reference state");
}
