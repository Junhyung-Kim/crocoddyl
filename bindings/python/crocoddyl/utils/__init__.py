...
class IntegratedActionModelRK4Derived(crocoddyl.ActionModelAbstract):
    ...
    def calc(self, data, x, u=None):
        ...
        if (self.enable_integration):
            data.dx = (data.ki[0] + 6. * data.ki[1] + 6. * data.ki[2] + data.ki[3]) * dt / 6
            data.xnext = self.differential.state.integrate(x, data.dx)
            data.cost = (data.int[0] + 6 * data.int[1] + 6 * data.int[2] + data.int[3]) * dt / 6
        else:
    ...
    def calcDiff(self, data, x, u):
        ...
        if (self.enable_integration):
            ...
            dxnext_dx, dxnext_ddx = self.state.Jintegrate(x, data.dx)
            ddx_dx = (data.dki_dx[0] + 6. * data.dki_dx[1] + 6. * data.dki_dx[2] + data.dki_dx[3]) * dt / 6
            data.ddx_du = (data.dki_du[0] + 6. * data.dki_du[1] + 6. * data.dki_du[2] + data.dki_du[3]) * dt / 6
            data.Fx[:] = dxnext_dx + np.dot(dxnext_ddx, ddx_dx)
            data.Fu[:] = np.dot(dxnext_ddx, data.ddx_du)

            data.Lx[:] = (data.dli_dx[0] + 6. * data.dli_dx[1] + 6. * data.dli_dx[2] + data.dli_dx[3]) * dt / 6
            data.Lu[:] = (data.dli_du[0] + 6. * data.dli_du[1] + 6. * data.dli_du[2] + data.dli_du[3]) * dt / 6

            data.Lxx[:] = (data.ddli_ddx[0] + 6. * data.ddli_ddx[1] + 6. * data.ddli_ddx[2] +
                           data.ddli_ddx[3]) * dt / 6
            data.Luu[:] = (data.ddli_ddu[0] + 6. * data.ddli_ddu[1] + 6. * data.ddli_ddu[2] +
                           data.ddli_ddu[3]) * dt / 6
            data.Lxu[:] = (data.ddli_dxdu[0] + 6. * data.ddli_dxdu[1] + 6. * data.ddli_dxdu[2] +
